...
a functor `Grothendieck.map : ∫F ⥤ Grothendieck G`.
-/
@[simps!]
def map (α : F ⟶ G) : ∫F ⥤ Grothendieck G where
  obj X :=
  { base := X.base
...
/-- The functor `Grothendieck.map α : ∫F ⥤ Grothendieck G` lies over `C`. -/
...
def compAsSmallFunctorEquivalenceInverse :
    ∫F ⥤ Grothendieck (F ⋙ Cat.asSmallFunctor.{w}) where
...
`Grothendieck (G ⋙ F) ⥤ ∫F`. -/
@[simps]
def pre (G : D ⥤ C) : Grothendieck (G ⋙ F) ⥤ ∫F where
...
-/
def preInv (G : D ≌ C) : ∫F ⥤ Grothendieck (G.functor ⋙ F) :=
...
`pre F G.functor` is an equivalence between `Grothendieck (G.functor ⋙ F)` and `∫F`.
-/
def preEquivalence (G : D ≌ C) : Grothendieck (G.functor ⋙ F) ≌ ∫F where
...
```
Grothendieck (G.functor ⋙ F) ≌ ∫F ⥤ ∫F' ≌ Grothendieck (G.functor ⋙ F')
```
-/
