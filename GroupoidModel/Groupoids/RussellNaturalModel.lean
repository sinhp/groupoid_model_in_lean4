...
@[simps] def catLift : Cat.{u,u} ⥤ Cat.{max v u,(max v u) + 1} where
  obj x := Cat.of (ULiftHom.{v} (ULift.{max v u + 1, u} x))
...
def groupoidNMBase0 : NaturalModelBase Ctx.{u} where
  Ty := Ty
  Tm := Tm
  tp := tp
  ext := ext
...
-- TODO link to `CategoryTheory.GroupidNMBase` in blueprint
def groupoidNMBase1 : NaturalModelBase Ctx.{u+1} where
  Ty := Ty
  Tm := Tm
  tp := tp
  ext := ext
...
#exit
def groupoidUHomSeq0 : NaturalModelBase.UHomSeq Ctx.{u} where
  length := 0
  objs i h := groupoidNMBase.{u}
  homSucc' i h := False.elim (by cases i; simp at h; simp at h)

def GroupoidUHomSeq1Objs (i : Nat) (h : i < 1) : NaturalModelBase Ctx.{u+1} :=
  match i with
  | 0 => groupoidNMBase.{u+1} -- bad
  | 1 => sorry
  | (n+2) => sorry

def groupoidUHomSeq1 : NaturalModelBase.UHomSeq Ctx.{u+1} where
  length := 1
  objs i h := sorry
  homSucc' i h := sorry
