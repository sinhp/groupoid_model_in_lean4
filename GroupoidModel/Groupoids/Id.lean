...
--   (h : α ⋙ forgetToGrpd = β ⋙ forgetToGrpd) (x : Γ)


-- def HomAtX := (α ⋙ forgetToGrpd).obj x ⟶ (β ⋙ forgetToGrpd).obj x


-- instance HomAtXCategory (x : Γ) : Category (@HomAtX _ _ α β x) := sorry

-- instance HomAtXGroupoid {α β : Γ ⥤ PGrpd.{v₁,u₁}}: Groupoid (@HomAtX _ _ α β x) :=
--   sorry
...
-- def smallUIdBase : NaturalModelIdBase smallU.{v} where
--   Id := smallUId.id
--   Refl := smallUId.refl
--   Id_comm := smallUId.comm
