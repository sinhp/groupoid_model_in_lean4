...
@[simp] theorem smallU_sec {Γ : Ctx.{max u (v+1)}} (α : y(Γ) ⟶ smallU.{v}.Tm) :
    smallU.sec α = Ctx.ofGrpd.map (sec (yonedaCategoryEquiv α)) := by
  ...
  . rw [NaturalModelBase.ym_sec_ym_disp]; rfl
...
@[simp] theorem base_sec {Γ : Ctx.{u}} (α : y(Γ) ⟶ base.Tm) :
    base.sec α = Ctx.ofGrpd.map (sec (yonedaCatEquiv α)) := by
  apply Yoneda.fullyFaithful.map_injective
  ...
  . rw [NaturalModelBase.sec_var]
    ...
    convert_to α =
    yonedaCatEquiv.symm
      (sec (yonedaCatEquiv α) ⋙ toPGrpd (yonedaCatEquiv α ⋙ Cat.homOf PGrpd.forgetToGrpd))
...
def baseUvPolyTpCompDomEquiv {Γ : Ctx.{u}} :
  ...
  convert_to _ ↔ yonedaCatEquiv (β ≫ yonedaCat.map PGrpd.forgetToGrpd)
    = Ctx.toGrpd.map (Ctx.ofGrpd.map
      (sec (yonedaCatEquiv α))) ⋙ yonedaCatEquiv B
  convert_to _ ↔ β ≫ yonedaCat.map PGrpd.forgetToGrpd =
    yoneda.map (Ctx.ofGrpd.map (sec (yonedaCatEquiv α))) ≫ B
...
def baseUvPolyTpCompDomEquiv' {Γ : Ctx.{u}} :
    ...
    ×' β ⋙ PGrpd.forgetToGrpd = sec α ⋙ map (eqToHom h) ⋙ B :=
