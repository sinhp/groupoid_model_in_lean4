...
--notation f " ≫ₗ " g => BasedLift.comp f g
end
...
(hv : v ≫ₗ g = g') : v = gaplift (g:= g) u g' := by
  simp [gaplift]; rw [← (Cartesian.uniq_lift (P:= P) (f:= f) (g:= g) (z:= x') u g').uniq ⟨v,hv⟩]

/-- A variant of the  uniqueness lemma. -/
@[simp]
lemma gaplift_uniq' {u : c' ⟶ c} (v : x' ⟶[u] x) (v' : x' ⟶[u] x) (hv : v ≫ₗ g = v' ≫ₗ g) : v = v' := by
  rw [gaplift_uniq g (v' ≫ₗ g) v hv]; symm; apply gaplift_uniq; rfl
...
have pbw' : P.map k.hom ≫ P.map f  = (u ≫ P.map f') ≫ P.map g := by
  rw [Category.assoc]; rw [u ≫= pbw.symm]; simp only [Fiber.tauto_over, over_base, eqToHom_refl, comp_id, id_comp, Category.assoc]
have hk : P.map k.hom = u ≫ P.map g' := by
  simp only [Fiber.tauto_over, over_base, eqToHom_refl, comp_id, id_comp, Category.assoc]
let v' :  w' ⟶ y := gapmap g gcart (u ≫ P.map f') (BasedLift.cast pbw' (k.cast (hk.symm) ≫ₗ tauto f))
have : k.hom ≫ f = v' ≫ g := by
  simp [v', gapmap_property,]
...
have hv₂ : P.map v ≫ P.map f' = u ≫ P.map f' := by
  rw [← P.map_comp, hv₂']
  simp only [gapmap_over]
have hv₃ : P.map v = u := by
  apply PullbackCone.IsLimit.hom_ext pb₂
  simp only [PullbackCone.mk_π_app_left]
  rw [hv₁, ← hu₁]
  simp only [PullbackCone.mk_π_app_right]
  rw [hv₂]
use ⟨v, by rw [hv₃]; simp⟩
--dsimp
constructor
