...
open CategoryTheory Limits Functor.Groupoidal
...
-/
@[simps] def U : RepMap.Universe repMap where
  Ty := Ty.{v}
  Tm := Tm.{v}
  tp := tp
  ext A := ext A
...
def asSmallClosedType' : (tensorUnit _ : Ctx)
    ⟶ Ty.{v+1, max u (v+2)} :=
...
def uHomSeqObjs (i : Nat) (h : i < 4) : RepMap.Universe repMap.{4} :=
  match i with
  | 0 => U.{0,4}
  | 1 => U.{1,4}
  | 2 => U.{2,4}
...
#exit
def smallUHom : NaturalModel.UHom smallU.{v, max u (v+2)} smallU.{v+1, max u (v+2)} :=
