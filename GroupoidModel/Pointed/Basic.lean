...
abbrev forgetToCat : PCat.{v, u} ⥤ Cat.{v, u} := Functor.Grothendieck.forget _
...
abbrev forgetToGrpd : PGrpd.{v, u} ⥤ Grpd.{v, u} := Functor.Grothendieck.forget _

/-- The forgetful functor from PGrpd to PCat -/
def forgetToPCat : PGrpd.{v, u} ⥤ PCat.{v, u} := pre (Functor.id Cat) forgetToCat
