...
@[simp] lemma id_fiber {C : PGrpd} : Grothendieck.Hom.fiber (𝟙 C) = 𝟙 _ := by
  rfl
...
@[simp] lemma map_base {C : Type u} [Category.{v} C] {F : C ⥤ PGrpd}
  {x y : C} (g : x ⟶ y) :
  (F.map g).base = forgetToGrpd.map (F.map g) := by
  rfl
