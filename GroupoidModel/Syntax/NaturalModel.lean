...
include r_tp in
def lift : y(Γ) ⟶ i.iFunctor.obj M.Tm :=
...
lemma equivFst_lift_eq : i.equivFst M (i.lift a C r r_tp) = a :=
  calc i.equivFst M (i.lift a C r r_tp)
  _ = i.equivFst M (i.lift a C r r_tp ≫ i.iFunctor.map M.tp) := by
...
-/
def j : y(i.motiveCtx a) ⟶ M.Tm :=
  eqToHom (by rw [equivFst_lift_eq]) ≫ i.equivSnd M (i.lift a C r r_tp)

/-- Typing for elimination rule `J` -/
lemma j_tp : j i a C r r_tp ≫ M.tp = C := by
  have := (UvPoly.Equiv.snd_naturality_right i.iUvPoly M.Tm M.Ty M.tp (i.lift a C r r_tp)).symm
  rw [eqToHom_comp_iff] at this
  simp only [j, Category.assoc, IdElimBase.equivSnd, this]
...
lemma reflSubst_j : ym(i.reflSubst a) ≫ j i a C r r_tp = r := by
  have h := i.equivSnd_verticalNatTrans_app M (i.lift a C r r_tp)
  ...
  rw! (castMode := .all) [equivFst_lift_eq, UvPoly.Equiv.fst_mk]
  simp only [j, eqToHom_refl, IsIso.inv_id, Category.id_comp, IsIso.inv_hom_id_assoc]
  congr 1
