...
structure UHom (M N : CwR.Universe) extends Hom M N where
  U : 𝟙_ Ctx ⟶ N.Ty
  asTm : M.Ty ⟶ N.Tm
  U_pb : IsPullback ((IsTerminal.ofUnique (𝟙_ Ctx)).from M.Ty) asTm U N.tp
    --         /- m.Ty -/           asTm /- N.Tm -/
    -- (isTerminal_yUnit.from M.Ty)
    --          /- ⊤ -/               U  /- N.Ty -/
...
def UHom.comp {M N O : CwR.Universe} (α : UHom M N) (β : UHom N O) : UHom M O where
  __ := Hom.comp α.toHom β.toHom
  U := α.U ≫ β.mapTy
  asTm := α.asTm ≫ β.mapTm
  U_pb := α.U_pb.paste_vert β.pb

def UHom.comp_assoc {M N O P : CwR.Universe} (α : UHom M N) (β : UHom N O) (γ : UHom O P) :
    comp (comp α β) γ = comp α (comp β γ) := by
  simp [comp, Hom.comp]

def UHom.wkU {M N : CwR.Universe} (Γ : Ctx) (α : UHom M N) : Γ ⟶ N.Ty :=
  (IsTerminal.ofUnique (𝟙_ Ctx)).from Γ ≫ α.U

@[reassoc (attr := simp)]
theorem UHom.comp_wkU {M N : CwR.Universe} {Δ Γ : Ctx} (α : UHom M N) (f : Δ ⟶ Γ) :
    f ≫ α.wkU Γ = α.wkU Δ := by
  simp [wkU]

/- Sanity check:
construct a `UHom` into a natural model with a Tarski universe. -/
def UHom.ofTarskiU (M : CwR.Universe) (U : 𝟙_ Ctx ⟶ M.Ty) (El : M.ext U ⟶ M.Ty) :
...
variable (s : UHomSeq CwR)

instance : GetElem (UHomSeq CwR) Nat (CwR.Universe) (fun s i => i < s.length + 1) where
...
def hom (s : UHomSeq CwR) (i j : Nat) (ij : i < j := by omega)
...
theorem mk_comp_cartesianNatTrans {i j ij jlen} {Γ X} (A : Γ ⟶ s[i].Ty)
    ...
    ((substCons _ (s[j].disp _) _ (s.unliftVar i j ij jlen A rfl) (by simp; sorry)) ≫ B) := by
...
theorem hom_comp_trans (s : UHomSeq CwR) (i j k : Nat) (ij : i < j) (jk : j < k)
    (klen : k < s.length + 1) :
...
Γ ---> 1 -----> s[i+1].Ty
              U_i
-/
def code {Γ : Ctx} {i : Nat} (s : UHomSeq CwR) (ilen : i < s.length) (A : Γ ⟶ s[i].Ty) :
    Γ ⟶ s[i+1].Tm :=
  A ≫ (s.homSucc i).asTm

@[simp]
theorem code_tp {Γ : Ctx} {i : Nat} (s : UHomSeq CwR) (ilen : i < s.length) (A : Γ ⟶ s[i].Ty) :
...
Γ ---> 1 -----> s[i+1].Ty
              U_i
-/
def el (s : UHomSeq CwR) {Γ : Ctx} {i : Nat} (ilen : i < s.length)
    (a : Γ ⟶ s[i+1].Tm) (a_tp : a ≫ s[i+1].tp = (s.homSucc i).wkU Γ) :
    Γ ⟶ s[i].Ty :=
  (s.homSucc i).U_pb.lift a (isTerminal_yUnit.from Γ) (by rw [a_tp, UHom.wkU])

@[reassoc]
theorem comp_el (s : UHomSeq CwR) {Δ Γ : Ctx} {i : Nat} (ilen : i < s.length)
...
@[simp]
lemma el_code {Γ : Ctx} {i : Nat} (s : UHomSeq CwR) (ilen : i < s.length) (A : Γ ⟶ s[i].Ty) :
    el s ilen (code s ilen A) (code_tp _ _ _) = A :=
  (s.homSucc i).U_pb.hom_ext (by simp [el, code]) (by simp)

@[simp]
lemma code_el (s : UHomSeq CwR) {Γ : Ctx} {i : Nat} (ilen : i < s.length)
    (a : Γ ⟶ s[i+1].Tm) (a_tp : a ≫ s[i+1].tp = (s.homSucc i).wkU Γ) :
...
Γ ⊢ₘₐₓ₍ᵢ,ⱼ₎ ΠA. B type
``` -/
protected class PiSeq (s : UHomSeq CwR) where
