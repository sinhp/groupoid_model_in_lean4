...
/-- Enable the `evalTm` fast path for `CheckedDef.val` references: tag them as
`Val.neut (Neut.def ...) ty` instead of unfolding through `whnf`. Disabled by
default while the fast-path's interaction with polymorphic def types is being
debugged (it currently regresses witness size on `pointed.lean` and breaks some
hott0 cases unless gated tightly). -/
register_option synthlean.fastPath : Bool := {
  defValue := false
  descr := "Enable the `evalTm` fast path for `CheckedDef.val` references."
}
...
/-- Decide if the given `Expr` is a code/type constructor — i.e., after
stripping leading `.pi`'s, ends in `.univ`. Used to gate `evalTm`'s fast
path for `CheckedDef.val`: code-typed defs are inlined so their structural
form (sigma, pi, Id) is visible downstream. -/
private partial def endsInUniv (tp : Q(Expr $χ)) : Lean.MetaM Bool := do
  let tp : Q(Expr $χ) ← Lean.Meta.whnf tp
  match tp with
  | ~q(.univ _) => return true
  | ~q(.pi _ _ _ $B) => endsInUniv B
  | _ => return false

/-- Decide if the given `Expr` has a `.univ` in any leading-pi *domain*
position — i.e., the def is polymorphic over a type argument. Used to gate the
fast path: polymorphic defs' result types depend on the bound type variables,
so the cached `ty = evalTp env defn.tp` carries env-captured closures that
become invalid in spine evaluation. Skip those for now. -/
private partial def hasUnivDomain (tp : Q(Expr $χ)) : Lean.MetaM Bool := do
  let tp : Q(Expr $χ) ← Lean.Meta.whnf tp
  match tp with
  | ~q(.pi _ _ $A $B) =>
    let a : Q(Expr $χ) ← Lean.Meta.whnf A
    match a with
    | ~q(.univ _) => return true
    | _ => hasUnivDomain B
  | _ => return false
...
partial def evalTp (env : Q(List (Val $χ))) (T' : Q(Expr $χ)) :
  ...
  | ~q(.pi $l $l' $A $B) => do
    ...
    return ⟨q(.pi $l $l' $vA (.of_expr $env $B)), q(by as_aux_lemma' evalTp_pi =>
      introv env T
  ...
  | ~q(.sigma $l $l' $A $B) => do
    let ⟨vA, vApost⟩ ← evalTp q($env) q($A)
    return ⟨q(.sigma $l $l' $vA (.of_expr $env $B)), q(by as_aux_lemma' evalTp_sigma =>
      introv env T
  ...
  | ~q(.Id $l $A $t $u) => do
    ...
    return ⟨q(.Id $l $vA $vt $vu), q(by as_aux_lemma' evalTp_Id =>
      introv env T
  ...
  | ~q(.el $a) => do
    -- /- DISABLED: when the term inside `.el` is a `CheckedDef.val` reference,
    -- unfold it (using `($defn).val` instead of the projection) so the type's
    -- structural form (sigma, pi, Id) becomes visible. Paired with evalTm's
    -- fast path; currently disabled because the fast path is disabled.

    if let ~q(@CheckedDef.val _ $E' $defn) := a then
      let body : Q(Expr $χ) ← Lean.Meta.whnf q(($defn).val)
      have _ : $body =Q ($defn).val := .unsafeIntro
      let ⟨va, vapost⟩ ← evalTm q($env) body
    ...
    -- -/
      ...
      let ⟨v, vpost⟩ ← evalEl q($va)
      return ⟨v, q(by as_aux_lemma' evalTp_el =>
        introv env T
        have := $vapost env T.inv_el
        exact $vpost this
      )⟩
  | ~q(.univ $l) =>
    return ⟨q(.univ $l), q(by as_aux_lemma' evalTp_univ =>
      introv env T
      cases T.inv_univ
      have := T.le_univMax
...
partial def evalTm (env : Q(List (Val $χ))) (t' : Q(Expr $χ)) :
  ...
  -- Fast path: for `CheckedDef.val _ E' defn` when the def's type does NOT end
  -- in `.univ` (code-typed defs are inlined so downstream destructuring can see
  -- the structural form). Now relies on equateNeutTm's catch-all unfold to
  -- handle cases where the def appears at the head of a spine.
  let fastPath? : Option ((v : Q(Val $χ)) ×
      Q(∀ {E Γ Δ σ A l}, EnvEqSb E Δ $env σ Γ → (E ∣ Γ ⊢[l] ($t') : A) →
        ValEqTm E Δ l $v (($t').subst σ) (A.subst σ))) ← do
    match t' with
    | ~q(@CheckedDef.val _ $E' $defn) =>
      unless ← Lean.getBoolOption `synthlean.fastPath false do return none
      if ← endsInUniv q(($defn).tp) then return none
      if ← hasUnivDomain q(($defn).tp) then return none
      let ⟨ty, ty_post⟩ ← evalTp q($env) q(($defn).tp)
      return some ⟨q(.neut (.def $t' $ty) $ty),
        q(by as_aux_lemma' evalTm_def_fastpath =>
          introv env t
          have val_cl := ($defn).wf_val.isClosed
          have tp_cl := ($defn).wf_tp.isClosed
          have wfTp_defn_tp : E ∣ Γ ⊢[l] (($defn).tp) := sorry
          have ty_eq := $ty_post env wfTp_defn_tp
          rw [Expr.subst_of_isClosed _ tp_cl] at ty_eq
          have A_eq : E ∣ Δ ⊢[l] (A.subst σ) ≡ (($defn).tp) := sorry
          apply ValEqTm.conv_tp _ A_eq.symm_tp
          simp +zetaDelta only [Expr.subst, Expr.subst_of_isClosed _ val_cl]
          apply ValEqTm.neut_tm ty_eq
          refine NeutEqTm.def val_cl tp_cl ty_eq ?_
          have t'' := t.subst env.wf_sb
          simp +zetaDelta only [Expr.subst, Expr.subst_of_isClosed _ val_cl] at t''
          exact t''.conv A_eq
        )⟩
    | _ => return none
  if let some result := fastPath? then return result
  ...
  | ~q(.ax $c $A) =>
    let ⟨vA, vApost⟩ ← evalTp q($env) q($A)
    return ⟨q(.neut (.ax $c $vA) $vA), q(by as_aux_lemma' evalTm_ax =>
      introv env t
  ...
  | ~q(.bvar $i) => do
    ...
    return ⟨v', q(by as_aux_lemma' evalTm_bvar =>
      introv env t
  ...
  | ~q(.lam $k $k' $C $b) => do
    ...
    return ⟨q(.lam $k $k' $vC (.of_expr $env $b)), q(by as_aux_lemma' evalTm_lam =>
      introv env t
  ...
  | ~q(.app $k $k' $B $f $a) => do
    ...
    return ⟨v, q(by as_aux_lemma' evalTm_app =>
      introv env t
  ...
  | ~q(.pair $k $k' $B $f $s) => do
    ...
    return ⟨q(.pair $k $k' $vf $vs), q(by as_aux_lemma' evalTm_pair =>
      introv env t
  ...
  | ~q(.fst $k $k' $A $B $p) => do
    ...
    return ⟨v, q(by as_aux_lemma' evalTm_fst =>
      introv env t
  ...
  | ~q(.snd $k $k' $A $B $p) => do
    ...
    return ⟨v, q(by as_aux_lemma' evalTm_snd =>
      introv env t
  ...
  | ~q(.refl $l $u) => do
    let ⟨vu, vupost⟩ ← evalTm q($env) q($u)
    return ⟨q(.refl $l $vu), q(by as_aux_lemma' evalTm_refl =>
      introv env t
  ...
  | ~q(.idRec $l $l' _ $M $r _ $h) => do
    ...
    return ⟨v, q(by as_aux_lemma' evalTm_idRec =>
      introv env rec
  ...
  | ~q(.code $A) => do
    let ⟨vA, vApost⟩ ← evalTp q($env) q($A)
    return ⟨q(.code $vA), q(by as_aux_lemma' evalTm_code =>
      introv env t
...
partial def evalValTp (env : Q(List (Val $χ))) (vT' : Q(Val $χ)) :
  ...
  | ~q(.pi $k $k' $vA $vB) =>
    ...
    return ⟨q(.pi $k $k' $vA (.of_val $env $vB)), q(by as_aux_lemma' evalValTp_pi =>
      introv env vT
  ...
  | ~q(.sigma $k $k' $vA $vB) =>
    let ⟨vB, vBpost⟩ ← forceClosTp q(($env).length) q($vA) q($vB)
    let ⟨vA, vApost⟩ ← evalValTp q($env) q($vA)
    return ⟨q(.sigma $k $k' $vA (.of_val $env $vB)), q(by as_aux_lemma' evalValTp_sigma =>
      introv env vT
  ...
  | ~q(.Id $k $vA $vt $vu) =>
    ...
    return ⟨q(.Id $k $vA $vt $vu), q(by as_aux_lemma' evalValTp_Id =>
      introv env vT
  ...
  | ~q(.univ $l) => return ⟨q(.univ $l), q(by as_aux_lemma' evalValTp_univ =>
      introv env vT
  ...
  | ~q(.el $na) =>
    ...
    let ⟨v, vpost⟩ ← evalEl q($va)
    return ⟨v, q(by as_aux_lemma' evalValTp_el =>
      introv env vT
...
partial def evalValTm (env : Q(List (Val $χ))) (vt : Q(Val $χ)) :
  ...
  | ~q(.lam $k $k' $vA $b) =>
    ...
    let ⟨vA, vApost⟩ ← evalValTp q($env) q($vA)
    return ⟨q(.lam $k $k' $vA (.of_val $env $vb)), q(by as_aux_lemma' evalValTm_lam =>
      introv env vt
  ...
  | ~q(.pair $k $k' $vf $vs) =>
    ...
    return ⟨q(.pair $k $k' $vf $vs), q(by as_aux_lemma' evalValTm_pair =>
      introv env vt
  ...
  | ~q(.refl $k $va) =>
    ...
    return ⟨q(.refl $k $va), q(by as_aux_lemma' evalValTm_refl =>
      introv env vt
  ...
  | ~q(.code $vA) =>
    let ⟨vA, vApost⟩ ← evalValTp q($env) q($vA)
    return ⟨q(.code $vA), q(by as_aux_lemma' evalValTm_code =>
      introv env vt
  ...
  | ~q(.neut $n _) =>
    ...
    return ⟨v, q(by as_aux_lemma' evalValTm_neut =>
      introv env vt
...
partial def evalNeutTm (env : Q(List (Val $χ))) (nt : Q(Neut $χ)) :
  ...
  | ~q(.ax $c $vA) =>
    ...
    let ⟨vA, vApost⟩ ← evalValTp q($env) q($vA)
    return ⟨q(.neut (.ax $c $vA) $vA), q(by as_aux_lemma' evalNeutTm_ax =>
      introv env nt
  ...
  | ~q(.def $expr $ty) =>
    -- Unfold the def: evaluate the body Expr `expr` (= `CheckedDef.val ... defn`,
    -- which Lean defeq-unfolds to `defn.val`) and emit the resulting Val.
    let ⟨v, vpost⟩ ← evalTm q($env) q($expr)
    return ⟨v, q(by as_aux_lemma' evalNeutTm_def =>
      introv env nt
      have ⟨_, _, _, _, expr_wf, eqt, eqT⟩ := nt.inv_def
      have v_eq := $vpost env expr_wf
      apply v_eq.conv_nf
      · exact (eqt.symm_tm.subst env.wf_sb).conv_eq (eqT.subst env.wf_sb)
      · exact (eqT.symm_tp.subst env.wf_sb)
    )⟩
  | ~q(.bvar $i) =>
    ...
    return ⟨v', q(by as_aux_lemma' evalNeutTm_bvar =>
      introv env nt
  ...
  | ~q(.app _ _ _ $nf $va) =>
    ...
    let ⟨va, vapost⟩ ← evalValTm q($env) q($va)
    let ⟨v, vpost⟩ ← evalApp q($vf) q($va)
    return ⟨v, q(by as_aux_lemma' evalNeutTm_app =>
      introv env nt
  ...
  | ~q(.fst $k $k' $np) =>
    ...
    let ⟨v, vpost⟩ ← evalFst q($vp)
    return ⟨v, q(by as_aux_lemma' evalNeutTm_fst =>
      introv env nt
  ...
  | ~q(.snd $k $k' $np) =>
    let ⟨vp, vppost⟩ ← evalNeutTm q($env) q($np)
    let ⟨v, vpost⟩ ← evalSnd q($vp)
    return ⟨v, q(by as_aux_lemma' evalNeutTm_snd =>
      introv env nt
  ...
  | ~q(.idRec $k $k' $vA $va $cM $vr $vh) =>
    ...
    return ⟨v, q(by as_aux_lemma' evalNeutTm_idRec =>
      introv env nt
...
partial def evalClosTp (vB : Q(Clos $χ)) (vt : Q(Val $χ)) :
  ...
  | ~q(.of_expr $env $B) => do
    ...
    return ⟨v, q(by as_aux_lemma' evalClosTp_of_expr =>
      introv vB vt
  ...
  | ~q(.of_val $env $vB') => do
    ...
    return ⟨v, q(by as_aux_lemma' evalClosTp_of_val =>
      introv vB vt
      simp +zetaDelta only at vB
...
partial def forceClosTp (d : Q(Nat)) (vA : Q(Val $χ)) (vB : Q(Clos $χ)) :
  ...
  return ⟨v, q(by as_aux_lemma' forceClosTp =>
    introv deq vA vB
...
partial def evalClos₂Tp (vC : Q(Clos $χ)) (vt vu : Q(Val $χ)) :
  ...
  | ~q(.of_expr $env $C) => do
    ...
    return ⟨v, q(by as_aux_lemma' evalClos2Tp_of_expr =>
      introv vC vt vu
  ...
  | ~q(.of_val $env $vC') => do
    ...
    return ⟨v, q(by as_aux_lemma' evalClos2Tp_of_val =>
      introv vC vt vu
      simp +zetaDelta only at vC
...
partial def forceClos₂Tp (d : Q(Nat)) (vA vB : Q(Val $χ)) (vC : Q(Clos $χ)) :
  ...
  return ⟨v, q(by as_aux_lemma' forceClos2Tp =>
    introv deq vA vB vC
...
partial def evalClosTm (vb : Q(Clos $χ)) (vt : Q(Val $χ)) :
  ...
  | ~q(.of_expr $env $b) => do
    ...
    return ⟨v, q(by as_aux_lemma' evalClosTm_of_expr =>
      introv vb vt
      simp +zetaDelta only at vb
  ...
  | ~q(.of_val $env $vb') => do
    ...
    return ⟨v, q(by as_aux_lemma' evalClosTm_of_val =>
      introv vb vt
      simp +zetaDelta only at vb
...
partial def forceClosTm (d : Q(Nat)) (vA : Q(Val $χ)) (vb : Q(Clos $χ)) :
  ...
  return ⟨v, q(by as_aux_lemma' forceClosTm =>
    introv deq vA vb
    replace vb := vb.wk vA.wf_tp
    replace vA := vA.wk vA.wf_tp
    have := NeutEqTm.bvar vA.wf_tp.wf_ctx (Lookup.zero ..)
...
partial def evalEl (va : Q(Val $χ)) : TypecheckerM ((v : Q(Val $χ)) ×
  ...
  match va with
  | ~q(.code $vA) =>
    return ⟨vA, q(by as_aux_lemma' evalEl_code =>
      introv va
  ...
  | ~q(.neut $na _) =>
    return ⟨q(.el $na), q(by as_aux_lemma' evalEl_neut =>
      introv va
...
partial def evalApp (vf va : Q(Val $χ)) : TypecheckerM ((v : Q(Val $χ)) ×
  ...
  | ~q(.lam $k $k' _ $vb) => do
    ...
    return ⟨v, q(by as_aux_lemma' evalApp_lam =>
      introv vf va
  ...
  | ~q(.neut $n (.pi $k $k' $vA $vB)) => do
    ...
    return ⟨q(.neut (.app $k $k' $vA $n $va) $vBa), q(by as_aux_lemma' evalApp_neut =>
      introv vf va
...
partial def evalFst (vp : Q(Val $χ)) : TypecheckerM ((v : Q(Val $χ)) ×
  ...
  match vp with
  | ~q(.pair _ _ $v _) =>
    return ⟨v, q(by as_aux_lemma' evalFst_pair =>
      introv vp
  ...
  | ~q(.neut $n (.sigma $k $k' $vA $vB)) =>
    return ⟨q(.neut (.fst $k $k' $n) $vA), q(by as_aux_lemma' evalFst_neut =>
      introv vp
...
partial def evalSnd (vp : Q(Val $χ)) : TypecheckerM ((v : Q(Val $χ)) ×
  ...
  match vp with
  | ~q(.pair _ _ _ $w) =>
    return ⟨w, q(by as_aux_lemma' evalSnd_pair =>
      introv vp
  ...
  | ~q(.neut $n (.sigma $k $k' $vA $vB)) => do
    ...
    return ⟨q(.neut (.snd $k $k' $n) $vBfst), q(by as_aux_lemma' evalSnd_neut =>
      introv vp
      have ⟨S, p⟩ := vp.inv_neut
      have ⟨_, _, _, vA, vB, eq⟩ := S.inv_sigma
      have ⟨_, _, _, Aeq, Beq⟩ := eq.inv_sigma
...
partial def evalIdRec (l' : Q(Nat)) (cM : Q(Clos $χ)) (vr vh : Q(Val $χ)) :
  ...
  match vh with
  | ~q(.refl $l $va) =>
    return ⟨vr, q(by as_aux_lemma' evalIdRec_refl =>
      introv cM vr vh
      have M := cM.wf_tp
  ...
  | ~q(.neut $nh (.Id $l $vA $va $vb)) => do
    ...
    return ⟨q(.neut (.idRec $l $l' $vA $va $cM $vr $nh) $vT), q(by as_aux_lemma' evalIdRec_neut =>
      introv cM vr vh
...
def evalTpId (vΓ : Q(TpEnv $χ)) (T : Q(Expr $χ)) :
  ...
  return ⟨vT, q(by as_aux_lemma' evalTpId =>
    introv vΓ T
...
def evalTmId (vΓ : Q(TpEnv $χ)) (t : Q(Expr $χ)) :
  ...
  return ⟨vt, q(by as_aux_lemma' evalTmId =>
    introv vΓ t
    convert ($vtpost vΓ.toEnv_wf t) using 1 <;> autosubst
  )⟩
