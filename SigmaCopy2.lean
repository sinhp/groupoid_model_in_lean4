...
def GetAlphaFromSection {Γ : Grpd}(A : Γ ⥤ Cat.of Grpd.{v₁,u₁})(B : Grothendieck.Groupoidal A ⥤ Grpd.{v₁,u₁})(sec : Γ ⥤ Grothendieck.Groupoidal (sigma A B)) : Γ ⥤ PGrpd := (SectionGrotSigmaToA A B sec) ⋙ Grothendieck.Groupoidal.toPGrpd _
...
def SectionGrotSigmaToB {Γ : Grpd}(A : Γ ⥤ Cat.of Grpd.{v₁,u₁})(B : Grothendieck.Groupoidal A ⥤ Grpd.{v₁,u₁})(sec : Γ ⥤ Grothendieck.Groupoidal (sigma A B)) : Γ ⥤ (Grothendieck.Groupoidal ((SectionGrotSigmaToA A B sec)⋙ B)) where
  ...
  map {x y} f := by
    fconstructor
    . exact f
    . refine eqToHom ?_ ≫ (sec.map f).fiber.fiber
      simp[Grpd.forgetToCat,SectionGrotSigmaToA,Grothendieck.Groupoidal.pre,ι,GrotSigmaToA,whiskerRight,map,Grothendieck.ιNatTrans]
      rw [<- Grpd.map_comp_obj]
      simp[CategoryStruct.comp,Grothendieck.comp]
      refine Functor.congr_obj ?_ _
...
def UP {Γ : Ctx} (top : yoneda.obj Γ ⟶ base.Tm) (left : yoneda.obj Γ ⟶ base.Ptp.obj base.{u}.Ty) (h : top ≫ base.tp = left ≫ baseSig) : yoneda.obj Γ ⟶ base.uvPolyTp.compDom base.uvPolyTp := by
  let AB := baseUvPolyTpEquiv.toFun (yonedaEquiv left)
  refine yonedaEquiv.invFun ?_
  refine baseUvPolyTpCompDomEquiv.invFun ?_
  refine ⟨?α,?B,?β,?h⟩
  let sec : (Ctx.toGrpd.obj Γ) ⥤ Grothendieck.Groupoidal (sigma AB.fst AB.snd) := by
    sorry
  . exact GetAlphaFromSection AB.1 AB.2 sec
  . sorry
  . sorry --exact GetBetaFromSection AB.1 AB.2 (sorry)
  . sorry
